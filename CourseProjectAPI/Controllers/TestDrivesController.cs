...
public class TestDrivesController : ControllerBase
{
    ...
    // Simple slot rules (MVP)
    private const int SlotDurationMinutes = 45;
    private const int WorkdayStartHourLocal = 10;
    private const int WorkdayEndHourLocal = 18; // last start is 17:15
    ...
    {
        // We treat the provided day as UTC day for simplicity in MVP.
        var day = (dateUtc ?? DateTime.UtcNow).Date;
        var start = new DateTime(day.Year, day.Month, day.Day, WorkdayStartHourLocal, 0, 0, DateTimeKind.Utc);
        var end = new DateTime(day.Year, day.Month, day.Day, WorkdayEndHourLocal, 0, 0, DateTimeKind.Utc);

        var slots = new List<TestDriveSlotDto>();
        for (var t = start; t.AddMinutes(SlotDurationMinutes) <= end; t = t.AddMinutes(SlotDurationMinutes))
        {
            slots.Add(new TestDriveSlotDto { StartsAtUtc = t, DurationMinutes = SlotDurationMinutes });
        }
    ...
    public async Task<ActionResult<BookTestDriveResponseDto>> Book([FromBody] BookTestDriveRequestDto dto)
    {
        try
        {
            ...
            var slotEnd = dto.StartsAtUtc.AddMinutes(SlotDurationMinutes);
